...
<template>
  <div id="blockShot">
    <ul class="contentList">
          <!-- Nba的盖帽 -->
          <li v-if="'/data/nba/blockshot' === dataMenuRoute">
            <table class="blockShotTable">
              <!-- 数据标题 -->
              <tr class="title">
                <td>排名</td>
                <td>球员</td>
              ...
              <!--  数据 -->
              <tr class="data" v-for="item in nbaBlockShotData">
                <td>{{ item.order }}</td>
                <td>{{ item.name }}</td>
                <td>{{ item.team }}</td>
                <td>{{ item.count }}</td>
          ...
          <!-- CBA联赛的盖帽 -->
          <li v-if="'/data/cba/blockshot' === dataMenuRoute">
            <table class="blockShotTable">
                <!-- 数据标题 -->
                <tr class="title">
                  <td>排名</td>
                ...
                <!--  数据 -->
                <tr class="data" v-for="item in cbaBlockShotData">
                  <td>{{ item.order }}</td>
                  <td>{{ item.name }}</td>
                  <td>{{ item.team }}</td>
                  <td>{{ item.count }}</td>
...
export default defineComponent({
  ...
  methods: {
    changeMenuDataCenter() {
      // 刷新后还是命中原来的的选项
      this.store.commit('handleDataCenter', 6);
    }
  }
})
</script>
